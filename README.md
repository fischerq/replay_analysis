...
Main classes lie in the package 'replay_analysis'.
...
A movement path is interpreted as a series of 3D points (x,y,t), that is then simplified using the [Ramer–Douglas–Peucker algorithm](http://en.wikipedia.org/wiki/Ramer%E2%80%93Douglas%E2%80%93Peucker_algorithm) to eliminate linear movement. 
...
The database schema is generated by the following SQL commands:
    ...
    CREATE TABLE IF NOT EXISTS Paths (path_id integer primary key, replay_id integer, unit_id integer, name text);
    CREATE TABLE IF NOT EXISTS PathNodes (node_id integer primary key, path integer, x integer, y integer, t real, duration real);
...
#Movement Graph
Aggregation of data by position. Not really good color scaling/ not legends yet(). 
